...
var swing_point = null
var swing_radius = null
const spring_constant = 15  # For spring effect when exceeding swing radius
const damping_factor = 0.1   # Damping factor for smoother movement
const upward_force = 30      # Upward force to enhance elevation during swing
...
func _physics_process(delta: float) -> void:
	...
	# Clamp motion based on max speed
	#var max_speed = is_on_floor() ? ground_max_speed : air_max_speed
	var max_speed = air_max_speed
	if motion_2d.length_squared() > max_speed * max_speed:
		motion_2d = motion_2d.normalized() * max_speed
...
func handle_hook(delta):
	...
	if swing_point == null:
		...
		# Apply upward force to the swing
		if point_distance > swing_radius:
			var excess_distance = point_distance - swing_radius
			motion -= point_normal * (spring_constant * excess_distance) * 0.1

			# Adding upward force when swinging
			motion.y += upward_force * delta  # Adjust this value to increase or decrease elevation
			
			# Reset position to swing radius
			global_transform.origin = swing_point + point_normal * swing_radius
			motion = motion.slide(point_normal)  # Keep the motion aligned with the swing
